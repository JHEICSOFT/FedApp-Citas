...
{
    public partial class RNBAJOPESO : Form
    {
        public RNBAJOPESO()
        {
            InitializeComponent();
        ...
        private void RNBAJOPESO_Load(object sender, EventArgs e)
        {
            ...
            if (cbambito.Text == "Disa")
            {
                ...
                else  //45 spacios segun combobox
                {
                    using (SqlConnection cnn = Conexion.ObternerConexion())
                    {
                        ...
                        string query = "SELECT count(*) from (SELECT CNV FROM DENOMINADORSII01 group by CNV)as t";
                        ...
                        string query2 = "SELECT count(*) from (SELECT CNV FROM BAJOPESO group by CNV)as t";
                        SqlCommand cmd3 = new SqlCommand(query2, cnn);
                        lblnumerador.Text = cmd3.ExecuteScalar().ToString();

                        // muestra el total del numerador en un label
                        string query3 = "SELECT count(*) from (select d.CNV from DENOMINADORSII01 as d LEFT join BAJOPESO as n ON D.CNV=n.CNV WHERE (n.CODIGO_ITEM IS NULL or n.CODIGO_ITEM='' ) group by d.CNV) as t";
        ...
        public void cargar_denominador(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    SqlDataAdapter da;
                    DataTable dt;
                    da = new SqlDataAdapter("select * from DENOMINADORSII01", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
        ...
        public void cargar_numerador(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    SqlDataAdapter da;
                    DataTable dt;
                    da = new SqlDataAdapter("select * from BAJOPESO", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
        ...
        public void falta(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    da = new SqlDataAdapter("select d.* from DENOMINADORSII01 as d LEFT join BAJOPESO as n ON D.CNV=n.CNV WHERE (n.CODIGO_ITEM IS NULL or n.CODIGO_ITEM='' )order by [NOMBRE DEL EESS]", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                    cnn.Close();
        ...
        {
            if (listAtenciones.Visible==true)
            {
                detalleAtencion();
            }
        ...
        private void listAtenciones_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
