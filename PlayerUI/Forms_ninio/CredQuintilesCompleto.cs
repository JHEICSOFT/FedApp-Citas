...
{
    public partial class CredQuintilesCompleto : Form
    {
        public CredQuintilesCompleto()
        {
            InitializeComponent();
            cargar_datosDisa();
        }
        ...
        private void CredQuintilesCompleto_Load(object sender, EventArgs e)
        {
            ...
            if (cbambito.Text == "Disa")
            {
                ...
                else  //45 spacios segun combobox
                {
                    using (SqlConnection cnn = Conexion.ObternerConexion())
                    {
                        DataTable dt = new DataTable();
                        string strSql;
                        strSql = "SP_CRED_COMPLETO_QUINTILESDISA";
                        ...
                        string query = "SELECT count(*) from (SELECT DOCIDENTIDAD FROM POBLACIONSMC02 group by DOCIDENTIDAD)as t";
                        ...
                        string query2 = "SELECT count(*) from (SELECT NUMERO_DOCUMENTO FROM NUMERADORMC02001 group by NUMERO_DOCUMENTO)as t";
                        SqlCommand cmd3 = new SqlCommand(query2, cnn);
                        lblnumerador.Text = cmd3.ExecuteScalar().ToString();

                        // muestra el total del numerador en un label
                        string query3 = "SELECT count(*) from (select d.DOCIDENTIDAD from POBLACIONSMC02 as d LEFT join NUMERADORMC02001 as n ON D.DOCIDENTIDAD=n.NUMERO_DOCUMENTO WHERE n.CODIGO_ITEM IS NULL group by d.DOCIDENTIDAD) as t";
            ...
            else if (cbambito.Text == "Red")
            {
                ...
                else  //45 spacios segun combobox
                {
                    using (SqlConnection cnn = Conexion.ObternerConexion())
                    {
                        DataTable dt = new DataTable();
                        string strSql;
                        strSql = "SP_CRED_COMPLETO_QUINTILESRED";
                        SqlDataAdapter da = new SqlDataAdapter(strSql, cnn);
            ...
            else if (cbambito.Text == "Microred")
            {
                ...
                else  //45 spacios segun combobox
                {
                    using (SqlConnection cnn = Conexion.ObternerConexion())
                    {
                        DataTable dt = new DataTable();
                        string strSql;
                        strSql = "SP_CRED_COMPLETO_QUINTILES";
                        SqlDataAdapter da = new SqlDataAdapter(strSql, cnn);
        ...
        public void cargar_denominador(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    SqlDataAdapter da;
                    DataTable dt;
                    da = new SqlDataAdapter("SELECT * FROM POBLACIONSMC02", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
        ...
        public void cargar_numerador(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    SqlDataAdapter da;
                    DataTable dt;
                    da = new SqlDataAdapter("SELECT * FROM NUMERADORMC02001", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
        ...
        public void falta(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    da = new SqlDataAdapter("select * from POBLACIONSMC02 as d LEFT join NUMERADORMC02001 as n ON D.DOCIDENTIDAD=n.NUMERO_DOCUMENTO WHERE n.CODIGO_ITEM IS NULL", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                    cnn.Close();
