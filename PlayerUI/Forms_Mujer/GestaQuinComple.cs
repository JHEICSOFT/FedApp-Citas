...
{
    public partial class GestaQuinComple : Form
    {
        public GestaQuinComple()
        {
            InitializeComponent();
            cargar_datosDisa();
        }
        ...
        private void GestaQuinComple_Load(object sender, EventArgs e)
        {
            ...
            if (cbambito.Text == "Disa")
            {
                ...
                else  //45 spacios segun combobox
                {
                    using (SqlConnection cnn = Conexion.ObternerConexion())
                    {
                        DataTable dt = new DataTable();
                        string strSql;
                        strSql = "SP_GESTAQUINTICOMPLEDISA";
                        ...
                        string query = "SELECT count(*) from (SELECT Documento FROM DENOMINADORMC01 group by Documento)as t";
                        ...
                        string query2 = "SELECT count(*) from (SELECT NUMERO_DOCUMENTO FROM GESTQUINCOMPLETO group by NUMERO_DOCUMENTO)as t";
                        ...
                        string query3 = "SELECT count(*) from (select d.Documento from DENOMINADORMC01 as d LEFT join GESTQUINCOMPLETO as n ON D.Documento=n.NUMERO_DOCUMENTO WHERE (n.CODIGO_ITEM IS NULL OR n.CODIGO_ITEM='') group by d.Documento) as t";
            ...
            else if (cbambito.Text == "Red")
            {
                ...
                else  //45 spacios segun combobox
                {
                    using (SqlConnection cnn = Conexion.ObternerConexion())
                    {
                        DataTable dt = new DataTable();
                        string strSql;
                        strSql = "SP_GESTAQUINTICOMPLERED";
                        SqlDataAdapter da = new SqlDataAdapter(strSql, cnn);
        ...
        public void cargar_denominador(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    SqlDataAdapter da;
                    DataTable dt;
                    da = new SqlDataAdapter("select * from DENOMINADORMC01", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
        ...
        public void cargar_numerador(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    SqlDataAdapter da;
                    DataTable dt;
                    da = new SqlDataAdapter("select * from GESTQUINCOMPLETO", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                }
        ...
        public void falta(DataGridView dgv)
        {
            try
            {
                using (SqlConnection cnn = Conexion.ObternerConexion())
                {
                    ...
                    da = new SqlDataAdapter("select D.* from DENOMINADORMC01 as d LEFT join GESTQUINCOMPLETO as n ON D.Documento=n.NUMERO_DOCUMENTO WHERE (n.CODIGO_ITEM IS NULL or n.CODIGO_ITEM='')", cnn);
                    dt = new DataTable();
                    da.Fill(dt);
                    dgv.DataSource = dt;
                    cnn.Close();
